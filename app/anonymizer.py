...
class AI():
  ...
  def run(self, img):
    if (img != None):
        ...
        use_appsrc = model.use_appsrc
        mirror = model.args.mirror
        videosrc = model.args.videosrc
        h264input = model.args.h264
        return(model.render_overlay(img), model.engine, model.src_size, model.appsink_size, use_appsrc, mirror, videosrc, h264input)
