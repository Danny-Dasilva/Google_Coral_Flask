...
def draw_pose(outputs, color='yellow', threshold=0.2):
    xys = {}
    for pose in outputs:
        for label, keypoint in pose.keypoints.items():

            if keypoint.score < threshold: continue
            print(int(keypoint.yx[1]))
...
def main():
    ...
    def render_overlay(engine, image, svg_canvas, height):
    ...
    gstreamer.run_pipeline(partial(render_overlay, engine),
